...
The browser also registers the Studio's bounded WebMCP tools on the current page. A local relay can expose those tools to a desktop MCP client, while the Project remains in the current visitor's browser session.

See [WebMCP agent tools](/webmcp) for the relay setup, tool catalog, approval boundary, and typed overhead/frame operations.

| Layer | Responsibilities |
|---|---|
| React and TanStack Start | Script, Stage, Frame, playback, editing, user controls and the page-side WebMCP registry |
